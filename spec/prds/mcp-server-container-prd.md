...
2. **Architectural Parity with Backend Microservice**:
   ...
   - **Tokio Runtime Metrics Telemetry (`tokio-metrics`)**: Integrates `tokio-metrics` with `metrics-rs-integration` to automatically sample and publish runtime scheduler metrics to Prometheus, ensuring parity with `aad-be-container`.
   - **Pod Endpoint Resolution for Telemetry**: Adheres to the shared test harness endpoint resolution abstraction, ensuring metrics-based integration tests query direct pod IPs (`http://<pod_ip>:8079/hams/metrics`) in-cluster while defaulting to loopback endpoints in local environments.
