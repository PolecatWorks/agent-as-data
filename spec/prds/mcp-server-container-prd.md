...
2. **Architectural Parity with Backend Microservice**:
   ...
   - **Tokio Runtime Metrics Telemetry**: Maintain a `tokio::runtime::Handle` in `AppState` and dynamically append Tokio runtime metrics (`tokio_workers_count`, `tokio_alive_tasks`, `tokio_blocking_threads`, `tokio_idle_blocking_threads`) to HaMS metrics scraping (`/hams/metrics`).
