...
- **Entity Selection Post-Deletion Stability Rule**: When an entity (Agent, Skill, Tool, Trait) is deleted from a registry view, the application MUST select the first available entity from the currently **filtered** list, not the raw underlying dataset. This prevents the selection jumping to an entity that is visually hidden by an active search query.
