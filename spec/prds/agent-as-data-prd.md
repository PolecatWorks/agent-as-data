...
- **Tokio Runtime Metrics Telemetry**: The application compiles with `--cfg tokio_unstable` and maintains a `tokio::runtime::Handle` in `AppState`. Prometheus exports dynamically append Tokio runtime metrics (`tokio_workers_count`, `tokio_alive_tasks`, `tokio_blocking_threads`, `tokio_idle_blocking_threads`) to both the HaMS closure scraper (`/hams/metrics`) and C-FFI callbacks, enabling detailed runtime scheduler visibility and resource monitoring.
