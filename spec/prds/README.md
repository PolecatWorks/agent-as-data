...
| [knowledge-data-system-prd.md](./knowledge-data-system-prd.md) | **Knowledge System PRD**: Long-term Project Memory, RAG vector chunks (`pgvector`), Graph Tuples (SPO), BREAD Operations UI, Knowledge Explorer ecosystem graph with relational tuple visualization, and native MCP tools. |
...
| [agent-ui-testing-kit-prd.md](./agent-ui-testing-kit-prd.md) | **Agent UI PRD**: Developer UI container (`aad-fe-container`) built with Angular 18+, TailwindCSS, and Angular Material for agent development, live testing, SSE token streaming, Mermaid network graph visualization, Knowledge Explorer with SPO tuple inspection, MCP server management, and Dual-Mode Telemetry Endpoint Resolution Abstraction (Local vs. In-Cluster Pod IPs). |
...
| [knowledge-mcp-journey-prd.md](./knowledge-mcp-journey-prd.md) | **Knowledge Base MCP Journey PRD**: Details BREAD operations and hybrid search tools exposed via Model Context Protocol (MCP) enabling agents to autonomously discover, read, curate, and maintain knowledge nodes. |
