...
import { of } from 'rxjs';
import {
  InteractiveTestingComponent,
  TestEntity,
} from './interactive-testing.component';
...
describe('InteractiveTestingComponent', () => {
  ...
  const mockAgents: Agent[] = [
    {
      ...
      description:
        'Reviews code for vulnerability flaws and injection vectors.',
      agent_definition:
        'You are a paranoid security auditor. Scan for OWASP top 10.',
  ...
  const mockSkills: Skill[] = [
    {
      ...
      definition:
        'Format all incoming text into { timestamp, level, message } JSON.',
      tags: ['logging', 'json'],
  ...
  it('should create and load initial entities', () => {
    ...
    expect(component.selectedEntity?.definition).toBe(
      'You are a paranoid security auditor. Scan for OWASP top 10.',
    );
  ...
  it('should select a skill entity and update context', () => {
    const skillEntity: TestEntity = component.allEntities.find(
      (e) => e.type === 'skill',
    )!;
  ...
  it('should execute prompt via Rig / Ollama and display final output', () => {
    spyOn(apiService, 'executeAgent').and.returnValue(
      of({
    ...
    expect(component.finalOutput).toBe(
      'No critical memory safety vulnerabilities detected.',
    );
    expect(component.executionOutput).toContain(
      'No critical memory safety vulnerabilities detected.',
    );
  ...
  it('should render the zero-footprint concept guide trigger and configuration in top bar', () => {
    const trigger = fixture.nativeElement.querySelector(
      '[data-testid="testing-concept-trigger"]',
    );
    expect(trigger).toBeTruthy();
    expect(trigger.textContent).toContain('What is Testing Studio?');

    expect(component.conceptGuideMappings.length).toBe(3);
    expect(component.conceptGuideMappings[0].title).toBe(
      '1. Prompt & Trait Inspector',
    );
