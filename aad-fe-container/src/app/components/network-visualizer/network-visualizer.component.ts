...
export class NetworkVisualizerComponent implements OnInit, AfterViewInit {
  ...
  loadAllData(): void {
    ...
    forkJoin({
      ...
      next: ({ agents, skills, traits, toolsList }) => {
        ...
        forkJoin({
          fullAgents: agentHydrate$,
          fullTraits: traitHydrate$,
        }).subscribe({
          next: ({ fullAgents, fullTraits }) => {
            this.agents = (fullAgents as any[]).filter(Boolean) as Agent[];
            this.traits = (fullTraits as any[]).filter(
              Boolean,
            ) as TraitContract[];
  ...
  handleRouteParams(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      if (type && id) {
        let entityToSelect = this.allEntities.find(
          (e) => e.type === type && (e.id === id || e.name === id),
        );
        if (entityToSelect) {
  ...
  private agentEntityKey(id: string): string {
    return `agent:${id}`;
  }
  private skillEntityKey(id: string): string {
    return `skill:${id}`;
  }
  private traitEntityKey(name: string): string {
    return `trait:${name}`;
  }
  private mcpEntityKey(id: string): string {
    return `mcp:${id}`;
  }
  ...
  buildMermaidCode(entity: SelectableEntity): string {
    ...
    const emitNode = (
      ...
      const styleWithRoot = isRoot
        ? style.replace('stroke-width:1.5px', `stroke-width:${strokeWidth}`)
        : style;
      ...
      if (key.startsWith('agent:')) {
        ...
        const matchedTrait = this.traits.find(
          (t) => t.name === traitName || t.id === traitName,
        );
    ...
    const emitEdge = (fromKey: string, toKey: string, label: string) => {
      ...
      edgeLines.push(
        `    ${this.nodeIdFor(fromKey)} -->|${label}| ${this.nodeIdFor(toKey)}`,
      );
    };
    ...
    while (queue.length > 0) {
      ...
      if (currentKey.startsWith('agent:')) {
        ...
        const agentDesc = agent
          ? `v${agent.current_version} - ${agent.description || ''}`
          : '';
        emitNode(
          currentKey,
          label,
        ...
        const skillDesc = skill
          ? `v${skill.current_version} - ${skill.description || ''}`
          : '';
        emitNode(
          currentKey,
          label,
        ...
        if (depth < this.traceDepth) {
          ...
          this.agents
            .filter((a) =>
              (a.implements_traits || []).some(
                (t) =>
                  t === traitKey ||
                  (trait && (t === trait.id || t === trait.name)),
              ),
            )
            .forEach((a) => {
              const k = this.agentEntityKey(a.id);
        ...
        const mcpDesc = mcp
          ? `${toolsLabel ? toolsLabel + ' - ' : ''}${mcp.description || ''}`
          : '';
        emitNode(
          currentKey,
          label,
