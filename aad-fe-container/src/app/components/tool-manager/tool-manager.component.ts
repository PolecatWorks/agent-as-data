...
export class ToolManagerComponent implements OnInit {
  ...
  loadServers(): void {
    this.apiService.getTools().subscribe({
      next: (servers) => {
        this.registeredServers = servers.map((s) => {
          ...
          if (s.endpoint_config) {
            ...
            if (s.endpoint_config.description)
      ...
      error: () => {
        this.snackBar.open(
          'Failed to load registered tools from backend, using stubs.',
          'Close',
          { duration: 3000 },
        );
        const routeId = this.route.snapshot.paramMap.get('id');
  ...
  getFilteredServers(): any[] {
    ...
    return this.registeredServers.filter(
      (s) =>
  ...
  syncServer(): void {
    ...
    this.apiService.syncTool(id).subscribe({
        ...
        this.apiService.getTools().subscribe({
          next: (tools) => {
            ...
            if (updated && updated.cached_capabilities) {
              this.selectedServer.cached_capabilities =
                updated.cached_capabilities;
              if (idx >= 0) {
                this.registeredServers[idx].cached_capabilities =
                  updated.cached_capabilities;
              }
            }
          },
      ...
      error: (err) => {
        this.isSyncing = false;
        this.snackBar.open(`Sync failed: ${err.message || err}`, 'Close', {
          duration: 4000,
        });
      },
    });
    ...
    this.apiService
      ...
      .subscribe({
        next: (res) => {
          ...
          const idx = this.registeredServers.findIndex(
            (s) => s.server_name === res.server_name,
          );
          if (idx >= 0) {
          ...
          this.snackBar.open(
            `Successfully registered tool: ${res.server_name}`,
            'Close',
            { duration: 3000 },
          );
        ...
        error: () => {
          ...
          const idx = this.registeredServers.findIndex(
            (s) => s.server_name === this.serverForm.server_name,
          );
          if (idx >= 0) {
          ...
          this.snackBar.open(
            `Registered tool: ${this.serverForm.server_name} (using stubs)`,
            'Close',
            { duration: 3000 },
          );
  ...
  deleteServer(): void {
    ...
    this.apiService.deleteTool(id).subscribe({
      ...
      error: (err: any) => {
        this.snackBar.open(
          `Failed to remove tool: ${err.message || err}`,
          'Close',
          { duration: 3000 },
        );
      },
