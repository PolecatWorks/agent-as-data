...
export class SkillsRegistryComponent implements OnInit {
    ...
    this.apiService.getTools().subscribe({
      next: (servers) => {
        this.allTools = (servers || []).map((s) => {
          ...
          const description =
            endpoint && endpoint.description ? endpoint.description : '';
          return {
            ...s,
  ...
  loadSkills(): void {
    this.apiService.getSkills().subscribe({
      ...
      error: (err) => {
        this.snackBar.open(
          `Error loading skills: ${err.message || err}`,
          'Close',
          { duration: 3000 },
        );
      },
  ...
  loadSkill(id: string): void {
    this.apiService.getSkill(id).subscribe({
      ...
      error: (err) => {
        this.snackBar.open(
          `Error fetching skill details: ${err.message || err}`,
          'Close',
          { duration: 3000 },
        );
      },
  ...
  selectSkill(skill: Skill, keepEdit = false): void {
    ...
    this.skillForm = {
      ...
      implements_traits: skill.implements_traits
        ? [...skill.implements_traits]
        : [],
    ...
    this.implementationStr = JSON.stringify(
  ...
  enableEdit(): void {
    ...
    if (this.selectedSkill) {
      this.router.navigate(['/skills', this.selectedSkill.id], {
  ...
  requestAiReview() {
    ...
    const payload = {
      ...
      reviewer_skill_id:
        this.selectedReviewerSkillId !== 'NONE'
          ? this.selectedReviewerSkillId
          : null,
    };
  ...
  acceptRewrite() {
    if (this.aiReviewResult?.suggested_rewrite) {
      ...
      this.aiReviewResult = null;
      this.snackBar.open('Rewrite applied. Please save to confirm.', 'Close', {
        duration: 3000,
      });
    }
  }
  ...
  saveSkill(): void {
    try {
      ...
      this.snackBar.open(`JSON Parse Error: ${e.message}`, 'Close', {
        duration: 3000,
      });
      return;
    }
    ...
    if (this.selectedSkill && this.selectedSkill.id) {
      this.apiService
        .updateSkill(this.selectedSkill.id, this.skillForm as Skill)
        .subscribe({
          next: (updated) => {
      ...
      this.apiService.createSkill(this.skillForm).subscribe({
        ...
        error: (err) => {
          this.snackBar.open(
            `Creation failed: ${err.message || err}`,
            'Close',
            { duration: 3000 },
          );
        },
  ...
  executeDelete(): void {
    if (this.selectedSkill && this.selectedSkill.id) {
      this.apiService.deleteSkill(this.selectedSkill.id).subscribe({
        ...
        error: (err) => {
          this.snackBar.open(
            `Deletion failed: ${err.message || err}`,
            'Close',
            { duration: 3000 },
          );
        },
  ...
  promoteToAgent(): void {
    if (this.selectedSkill && this.selectedSkill.id) {
      this.apiService.promoteSkill(this.selectedSkill.id).subscribe({
        next: (agent) => {
          this.snackBar.open(
            `Successfully promoted to Agent: ${agent.name}`,
            'Close',
            { duration: 3000 },
          );
  ...
  getAvailableSkillsToAttach(): Skill[] {
    return this.skills.filter(
      (s) =>
        s.id !== this.selectedSkill?.id &&
        !(this.skillForm.attached_skills || []).includes(s.id || ''),
    );
  }

  getAvailableToolsToAttach(): any[] {
    return this.allTools.filter(
      (m) => !(this.skillForm.attached_tools || []).includes(m.id),
    );
  }
  ...
  detachSkill(id: string): void {
    if (this.skillForm.attached_skills) {
      this.skillForm.attached_skills = this.skillForm.attached_skills.filter(
        (i) => i !== id,
      );
    }
  }
  ...
  detachTool(id: string): void {
    if (this.skillForm.attached_tools) {
      this.skillForm.attached_tools = this.skillForm.attached_tools.filter(
        (i) => i !== id,
      );
    }
  }
  ...
  attachTraitFromCatalog(trait: string): void {
    if (!this.skillForm.implements_traits)
      this.skillForm.implements_traits = [];
  ...
  removeTrait(trait: string): void {
    if (this.skillForm.implements_traits) {
      this.skillForm.implements_traits =
        this.skillForm.implements_traits.filter((t: string) => t !== trait);
    }
  }
  ...
  syncEmbeddings() {
    if (this.selectedSkill?.id) {
      this.apiService
        .syncEmbeddings('skills', this.selectedSkill.id)
        .subscribe({
          next: (res) => {
            this.snackBar.open(
