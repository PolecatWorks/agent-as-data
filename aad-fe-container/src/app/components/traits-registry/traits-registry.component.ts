...
export class TraitsRegistryComponent implements OnInit {
  ...
  readonly conceptGuideMappings: ConceptTabMapping[] = [
    ...
    },
    {
      ...
      description:
        'Unbreakable corporate policy rules the agent MUST ALWAYS or MUST NEVER violate.',
    },
    {
      icon: 'fact_check',
          ...
          active_guardrails: [
            {
              ...
              tier: 'Tier 1: Deterministic',
              description:
                'Reject exact matching phrases or regex keywords in prompts',
              config: {
                blocked_input_keywords: ['ignore previous instructions'],
              },
            },
          ],
        },
  ...
  loadTraits(): void {
    this.apiService.getTraits().subscribe({
          ...
          (forkJoin(obs) as any).subscribe({
            next: (fullTraits: any[]) => {
              ...
              if (routeId) {
                ...
                !this.isEditing &&
                !isEdit &&
                !this.selectedTraitContract
              ) {
                if (this.filteredTraitContracts.length > 0) {
                  this.selectTraitContract(this.filteredTraitContracts[0]);
                }
              }
            },
            error: () => {
              this.snackBar.open(
                'Failed to load details for traits.',
                'Close',
                { duration: 3000 },
              );
            },
      ...
      error: () => {
        this.snackBar.open(
          'Failed to load traits from backend, using catalog stubs.',
          'Close',
          { duration: 3000 },
        );
  ...
  get filteredTraitContracts(): TraitContract[] {
    ...
    return this.traitContracts.filter(
      (t) =>
  ...
  selectTraitContract(trait: TraitContract, keepEdit = false): void {
    ...
    this.traitForm = {
      ...trait,
      capability_requirements: trait.capability_requirements
        ? [...trait.capability_requirements]
        : [],
      behavioral_invariants: trait.behavioral_invariants
        ? [...trait.behavioral_invariants]
        : [],
      evaluation_criteria: trait.evaluation_criteria
        ? [...trait.evaluation_criteria]
        : [],
  ...
  saveTraitContract(): void {
    ...
    if (
      ...
      this.apiService
        .updateTrait(this.selectedTraitContract.id, this.traitForm)
        .subscribe({
          next: (updated) => {
            const idx = this.traitContracts.findIndex(
              (t) => t.id === updated.id,
            );
            if (idx >= 0) {
            ...
            this.snackBar.open(
              `Updated trait ${updated.name} (v${updated.version})`,
              'Close',
              { duration: 3000 },
            );
          ...
          error: (err: any) => {
            this.snackBar.open(
              `Failed to save trait contract: ${err.message || err}`,
              'Close',
              { duration: 3000 },
            );
          },
      ...
      this.apiService.createTrait(this.traitForm).subscribe({
        next: (created) => {
          ...
          if (
            ...
            this.traitContracts = this.traitContracts.filter(
              (t) => t.id !== this.selectedTraitContract!.id,
            );
          }
          this.traitContracts.push(created);
          this.snackBar.open(`Created new trait ${created.name}`, 'Close', {
            duration: 3000,
          });
        ...
        error: (err: any) => {
          this.snackBar.open(
            `Failed to create trait contract: ${err.message || err}`,
            'Close',
            { duration: 3000 },
          );
        },
  ...
  deleteTraitContract(): void {
    ...
    if (id.startsWith('trait-')) {
      ...
      this.snackBar.open('Removed temporary trait definition', 'Close', {
        duration: 3000,
      });
      return;
    }
    ...
    this.apiService.deleteTrait(id).subscribe({
      next: () => {
        ...
        this.snackBar.open('Trait contract deleted successfully', 'Close', {
          duration: 3000,
        });
      },
      error: (err: any) => {
        this.snackBar.open(
          `Failed to delete trait contract: ${err.message || err}`,
          'Close',
          { duration: 3000 },
        );
      },
