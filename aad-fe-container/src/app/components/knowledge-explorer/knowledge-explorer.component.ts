...
export class KnowledgeExplorerComponent implements OnInit, AfterViewInit {
  ...
  readonly conceptGuideMappings: ConceptTabMapping[] = [
    {
      ...
      description: 'Interactive graph nodes representing Agents, Skills, Tools, Traits, and Knowledge documents.'
    },
    {
      icon: 'sync_alt',
      iconColor: 'text-amber-600',
      title: '2. Relational Tuples (SPO Triples)',
      description: 'Edges represent directional Subject-Predicate-Object relations (e.g., uses_tool, has_skill, implements).'
    },
    {
      icon: 'account_tree',
      iconColor: 'text-emerald-600',
      title: '3. Multi-Hop Graph Traversal',
      description: 'Explore multi-step dependencies and navigate directly from selected entities to connected dependencies.'
    }
  ];
  ...
  loadData(): void {
    forkJoin({
      ...
      traits: this.apiService.getTraits().pipe(catchError(() => of({ items: [] } as any))),
      knowledge: this.apiService.getKnowledgeNodes().pipe(catchError(() => of([])))
    }).pipe(
      switchMap(({ agentsSummary, skills, tools, traits, knowledge }) => {
        ...
        const agentsObs = agentHydrations.length > 0 ? forkJoin(agentHydrations) : of([] as Agent[]);

        // Fetch tuples for knowledge nodes in parallel
        const knList = (knowledge as KnowledgeNode[]) || [];
        const knTupleObs = knList.map(kn =>
          this.apiService.getKnowledgeTuples(kn.id).pipe(catchError(() => of([])))
        );
        const tuplesObs = knTupleObs.length > 0 ? forkJoin(knTupleObs) : of([]);
        ...
        const traitsObs = traitHydrations.length > 0
          ? forkJoin(traitHydrations).pipe(
              map(fetched => [...directTraits, ...fetched.filter(Boolean)])
            )
          : of(directTraits);
      ...
      const entityById = new Map<string, { id: string; name: string; type: any; raw: any }>();
      const entityByName = new Map<string, { id: string; name: string; type: any; raw: any }>();

      const registerEntity = (id: string, name: string, type: any, raw: any) => {
      ...
      results.tools.forEach((tool: any) => {
        ...
        if (tool.server_name) {
          entityByName.set(tool.server_name.toLowerCase(), { id: toolId, name: toolName, type: 'tool', raw: tool });
        }
        newNodes.push({
          id: toolId,
          label: toolName,
      ...
      const addTuple = (
        ...
        const exists = extractedTuples.some(
          t => t.subjectId === subject.id && t.predicate === predicate && t.objectId === object.id
        );
        if (!exists) {
      ...
      results.agents.forEach(agent => {
        const subject = { id: agent.id, name: agent.name, type: 'agent' as const };
        ...
        if (agent.implements_traits) {
          agent.implements_traits.forEach(traitRef => {
            ...
            if (!target && traitRef) {
              ...
              registerEntity(tid, traitRef, 'trait', { id: tid, name: traitRef, type: 'trait' });
              newNodes.push({
                id: tid,
                label: traitRef,
                color: { background: '#d1fae5', border: '#10b981', highlight: { background: '#a7f3d0', border: '#059669' } },
      ...
      results.skills.forEach(skill => {
        const skillId = skill.id || skill.name;
        const subject = { id: skillId, name: skill.name, type: 'skill' as const };
        ...
        results.knowledgeTuplesList.forEach((tuplesForNode: any[]) => {
          if (Array.isArray(tuplesForNode)) {
            tuplesForNode.forEach(kt => {
              const sourceNode = resolveEntity(kt.source_node_id) || resolveEntity(kt.subject);
              let targetNode = resolveEntity(kt.object);
              if (!targetNode && kt.object) {
                const conceptId = `concept-${kt.object.toLowerCase().replace(/\s+/g, '-')}`;
                registerEntity(conceptId, kt.object, 'knowledge', { id: conceptId, name: kt.object, type: 'knowledge' });
                newNodes.push({
                  id: conceptId,
                  label: kt.object,
                  shape: 'box',
                  color: { background: '#f8fafc', border: '#94a3b8', highlight: { background: '#e2e8f0', border: '#64748b' } },
                  rawData: { id: conceptId, name: kt.object, title: kt.object, type: 'knowledge' }
                });
                targetNode = resolveEntity(conceptId);
              }
              if (sourceNode && targetNode && sourceNode.id !== targetNode.id) {
                addTuple(sourceNode, kt.predicate || 'relates_to', targetNode, kt.confidence);
              }
            });
          }
        });
      ...
      this.availablePredicates = ['all', ...Object.keys(counts).filter(k => k !== 'all')];
  ...
  onNodeSelected(nodeId: string): void {
    ...
    if (node) {
      ...
      this.selectedNode = entity ? entity.raw : { id: nodeId, name: nodeId, type: 'unknown' };
    }
