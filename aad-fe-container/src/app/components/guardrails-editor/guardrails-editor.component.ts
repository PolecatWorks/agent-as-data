...
export class GuardrailsEditorComponent implements OnInit {
  ...
  guardrailCatalog = [
    {
      ...
      description:
        'Real-time heuristic scanning to intercept injection signatures before reaching LLM',
    },
    {
      type: 'pii_regex',
      ...
      tier: 'Tier 1: Deterministic',
      description:
        'Regex scanning to block SSNs, emails, credit cards, and secret credentials',
    },
    {
      type: 'max_input_tokens',
      name: 'Max Input Token Limit Cap',
      tier: 'Tier 1: Deterministic',
      description: 'Enforce hard upper limit cap on incoming prompt tokens',
    },
    {
    ...
    },
    {
      ...
      name: 'Vector Attack Similarity Matcher',
      tier: 'Tier 2: Vector Matching',
      description:
        'Cosine similarity matching against vector database of known jailbreaks',
    },
    {
      type: 'classifier_model',
      name: 'Specialized Safety Classifier Model',
      tier: 'Tier 3: Dedicated Classifier',
      description:
        'Route input to Llama Guard, DeBERTa v3, or Perspective API classifier',
    },
    {
      type: 'llm_judge',
      name: 'LLM-as-a-Judge Pre-Evaluator',
      tier: 'Tier 4: LLM Judge',
      description:
        'Run lightweight fast LLM evaluator against custom policy rules',
    },
    {
      type: 'domain_scoping',
      name: 'System Prompt Domain Scoping',
      tier: 'Tier 5: System Rules',
      description: 'Inject allowed topic constraints into system prompt',
    },
  ];
  ...
  outputGuardrailCatalog = [
    {
      ...
      category: 'Data Safety & Privacy',
      description:
        'Automatically scan and mask credentials, RSA private keys, and API tokens in responses',
    },
    {
      type: 'pii_ner_redaction',
      name: 'PII & Sensitive Data Redaction (NER)',
      category: 'Data Safety & Privacy',
      description:
        'Mask names, emails, phone numbers, SSNs, and credit cards using Named Entity Recognition (Presidio)',
    },
    {
      ...
      category: 'Data Safety & Privacy',
      description:
        'Redact internal stack traces, cluster IPs, private domain names, and database URIs',
    },
    {
      ...
      description:
        'Validate output against formal JSON Schema contracts prior to returning payload',
    },
    {
      type: 'max_output_tokens',
      name: 'Max Output Token Generation Limit',
      category: 'Generation Boundaries',
      description:
        'Hard token limit cap on total output generation per request',
    },
    {
      type: 'blocked_output_keywords',
      name: 'Blocked Output Keyword & Secret Redaction',
      category: 'Data Safety & Privacy',
      description:
        'Banned terms or secret pattern blacklists redacted from LLM output',
    },
    {
      type: 'toxicity_classifier',
      name: 'Post-Execution Toxicity & Harm Classifier',
      category: 'Safety & Policy',
      description:
        'Filter hate speech, harassment, or explicit material using Llama Guard or Perspective API',
    },
    {
      type: 'brand_competitor_protection',
      name: 'Competitor & Brand Protection Filter',
      category: 'Safety & Policy',
      description:
        'Redact or restrict mentions of banned competitor brands or restricted products',
    },
    {
      type: 'rag_grounding_hallucination',
      name: 'RAG Grounding & Hallucination Scoring (NLI)',
      category: 'Truthfulness & Quality',
      description:
        'Natural Language Inference grounding checks against retrieved context chunks',
    },
    {
      type: 'refusal_offtopic_detector',
      name: 'Refusal & Off-Topic Detector',
      category: 'Truthfulness & Quality',
      description: 'Detect hallucinated refusals or domain boundary drift',
    },
    {
      ...
      category: 'Quality & Structure',
      description:
        'Validate Markdown formatting, custom code block structures, or mandatory URL patterns',
    },
  ];
  ...
  addGuardrailFromDropdown(): void {
    ...
    const catItem = this.guardrailCatalog.find(
      (g) => g.type === this.selectedGuardrailTypeToAdd,
    );
    if (!catItem) return;
    ...
    this.snackBar.open(`Added ${catItem.name} guardrail`, 'Close', {
      duration: 2500,
    });
  }
  ...
  deleteGuardrail(id: string): void {
    if (this.guardrails?.input_guardrails?.active_guardrails) {
      this.guardrails.input_guardrails.active_guardrails =
        this.guardrails.input_guardrails.active_guardrails.filter(
          (g) => g.id !== id,
        );
      this.guardrailsChange.emit(this.guardrails);
      this.snackBar.open('Input guardrail removed', 'Close', {
        duration: 2000,
      });
    }
  }
  ...
  addOutputGuardrailFromDropdown(): void {
    ...
    const catItem = this.outputGuardrailCatalog.find(
      (g) => g.type === this.selectedOutputGuardrailTypeToAdd,
    );
    if (!catItem) return;
    ...
    this.guardrailsChange.emit(this.guardrails);
    this.snackBar.open(`Added ${catItem.name} output guardrail`, 'Close', {
      duration: 2500,
    });
  }
  ...
  deleteOutputGuardrail(id: string): void {
    if (this.guardrails?.output_guardrails?.active_guardrails) {
      this.guardrails.output_guardrails.active_guardrails =
        this.guardrails.output_guardrails.active_guardrails.filter(
          (g) => g.id !== id,
        );
      this.guardrailsChange.emit(this.guardrails);
      this.snackBar.open('Output guardrail removed', 'Close', {
        duration: 2000,
      });
    }
  }
  ...
  private getDefaultConfigForType(type: string): any {
    switch (type) {
      ...
      case 'llm_judge':
        return {
          judge_model: 'gpt-4o-mini-v2024-07-18',
          judge_custom_policy_prompt:
            'Verify input adheres to safety guidelines.',
        };
      case 'domain_scoping':
  ...
  private getDefaultOutputConfigForType(type: string): any {
    switch (type) {
      ...
      case 'pii_ner_redaction':
        return {
          pii_ner_entities: ['EMAIL', 'PHONE_NUMBER', 'SSN', 'CREDIT_CARD'],
        };
      ...
      case 'blocked_output_keywords':
        return {
          blocked_output_keywords: ['AWS_SECRET_ACCESS_KEY', 'PRIVATE KEY'],
        };
      case 'toxicity_classifier':
        return { classifier_type: 'llama_guard', toxicity_threshold: 0.7 };
      case 'brand_competitor_protection':
        return {
          banned_competitor_brands: ['CompetitorX', 'RestrictedProductY'],
        };
  ...
  addKeywordToGuardrail(g: any, inputEl: HTMLInputElement): void {
    ...
    if (val) {
      if (!g.config.blocked_input_keywords)
        g.config.blocked_input_keywords = [];
  ...
  removeKeywordFromGuardrail(g: any, kw: string): void {
    if (g.config.blocked_input_keywords) {
      g.config.blocked_input_keywords = g.config.blocked_input_keywords.filter(
        (k: string) => k !== kw,
      );
      this.guardrailsChange.emit(this.guardrails);
    }
  ...
  removeTopicFromGuardrail(g: any, topic: string): void {
    if (g.config.allowed_domain_topics) {
      g.config.allowed_domain_topics = g.config.allowed_domain_topics.filter(
        (t: string) => t !== topic,
      );
      this.guardrailsChange.emit(this.guardrails);
    }
  ...
  removePiiEntityFromGuardrail(g: any, entity: string): void {
    if (g.config.pii_ner_entities) {
      g.config.pii_ner_entities = g.config.pii_ner_entities.filter(
        (e: string) => e !== entity,
      );
      this.guardrailsChange.emit(this.guardrails);
    }
  ...
  removeInfraLeakTypeFromGuardrail(g: any, type: string): void {
    if (g.config.infra_leak_types) {
      g.config.infra_leak_types = g.config.infra_leak_types.filter(
        (t: string) => t !== type,
      );
      this.guardrailsChange.emit(this.guardrails);
    }
  ...
  addBannedBrandToGuardrail(g: any, inputEl: HTMLInputElement): void {
    ...
    if (val) {
      if (!g.config.banned_competitor_brands)
        g.config.banned_competitor_brands = [];
  ...
  removeBannedBrandFromGuardrail(g: any, brand: string): void {
    if (g.config.banned_competitor_brands) {
      g.config.banned_competitor_brands =
        g.config.banned_competitor_brands.filter((b: string) => b !== brand);
      this.guardrailsChange.emit(this.guardrails);
    }
  }
  ...
  addBlockedOutputKeywordToGuardrail(g: any, inputEl: HTMLInputElement): void {
    ...
    if (val) {
      if (!g.config.blocked_output_keywords)
        g.config.blocked_output_keywords = [];
  ...
  removeBlockedOutputKeywordFromGuardrail(g: any, kw: string): void {
    if (g.config.blocked_output_keywords) {
      g.config.blocked_output_keywords =
        g.config.blocked_output_keywords.filter((k: string) => k !== kw);
      this.guardrailsChange.emit(this.guardrails);
    }
  }
  ...
  removeCustomRegexRuleFromGuardrail(g: any, rule: string): void {
    if (g.config.custom_regex_rules) {
      g.config.custom_regex_rules = g.config.custom_regex_rules.filter(
        (r: string) => r !== rule,
      );
      this.guardrailsChange.emit(this.guardrails);
    }
