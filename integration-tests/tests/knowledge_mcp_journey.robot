...
Test Knowledge Base MCP Journey
    ...
    ${response}=   POST On Session      backend    ${API_PREFIX}/knowledge    json=${payload}    headers=${headers}
    ...
    # Step 2: Read Node
    ${response}=   GET On Session       backend    ${API_PREFIX}/knowledge/${CREATED_NODE_ID}
    ...
    # Step 3: Browse Nodes
    ${response}=   GET On Session       backend    ${API_PREFIX}/knowledge
    Should Be Equal As Strings    ${response.status_code}    200
    ${body}=       Set Variable    ${response.json()}
    ...
    ${response}=   POST On Session      backend    ${API_PREFIX}/knowledge/search    json=${search_payload}    headers=${headers}
    Should Be Equal As Strings    ${response.status_code}    200
    ${body}=       Set Variable    ${response.json()}
    ...
    ${response}=   PUT On Session       backend    ${API_PREFIX}/knowledge/${CREATED_NODE_ID}    json=${edit_payload}    headers=${headers}
    Should Be Equal As Strings    ${response.status_code}    200
...
Cleanup Knowledge Node
    ...
    Run Keyword And Ignore Error    Run Keyword If    '${CREATED_NODE_ID}' != '${EMPTY}'    DELETE On Session    backend    ${API_PREFIX}/knowledge/${CREATED_NODE_ID}
